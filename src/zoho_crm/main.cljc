(ns zoho_crm.main
  "Kotodama WASM entrypoint for the Zoho_crm clean-room actor (L4) — Clojure port.
  ...
  facts (`zoho_crm.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "zoho_crm")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"zoho_crm.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zoho_crm-compat" :tier tier :entities entities} 200])
...
(defn start! [] :zoho_crm-compat/ready)
